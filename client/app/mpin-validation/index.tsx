...
export default function MPINValidationScreen() {
  ...
  const handleMpinSubmit = async (mpinValue?: string) => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.1.100:3001/api/auth/validate-mpin', {
        method: 'POST',
        headers: {
      ...
      // ✅ Successful MPIN validation - proceed to typing game
  ...
  const handleBackspace = () => {
    const lastFilledIndex = mpin.map((digit, index) => digit ? index : -1)
        ...
        <View style={styles.content}>
          ...
          <View style={styles.footer}>
            <TouchableOpacity 
              ...
              onPress={() => Alert.alert('Help', 'Contact customer support for MPIN assistance')}
            >
